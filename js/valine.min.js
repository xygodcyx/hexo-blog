		...
		, function (e, t, n) {
			...
			t.DEFAULT_EMOJI_CDN = "//img.t.sinajs.cn/t4/appstyle/expression/ext/normal/",
				...
				t.CONFIG = {
					...
					placeholder: "可以输入注册https://gravatar.com/时的邮箱以显示在gravatar中保存的头像（输入邮箱还能接收回复提醒哦）",
					pageSize: 20,
					recordIP: !0,
					serverURLs: "",
					visitor: !1,
		...
		, function (e, t, n) {
			...
			var r = n(3)
				, i = {
					...
					build: function (index, e, focusCdn, t) {
						var n = /^(https?:)?\/\//i
							, r = i.maps[index]
							, o = n.test(r) ? r : (focusCdn ? focusCdn : i.cdn) + r
				...
				i.prototype._init = function () {
					...
					try {
						...
						var p = O.ds
							, f = r ? "&q=" + (0,
								h.RandomStr)() : "";
						O.params = "?d=" + (p.indexOf(n) > -1 ? n : "monsterid") + "&v=" + h.VERSION + f,
							O.hide = "hide" === n,
				...
				i.prototype.bind = function () {
					...
					var e = this
						...
						, c = function (e) {
							...
							for (var r in a) {
								var title = a[r].split("/")[0]
								if (title === "hr") {
									hrCount.push(r)
								}
								if (!a[r].includes("/"))
									continue
								if (title !== "hr" && titles.findIndex((item) => item.title === title) === -1)
									titles.push({ title, hasPush: false, range: [] })
							}
							titles = titles.map((title, i) => {
								var start = a.findIndex((r) => r.includes(title.title))
								title.range = [start, i < hrCount.length ? +hrCount[i] : a.length]
								return title
							})
							let titleIndex = 0
							console.log(titles)
							titles.forEach((title) => {
								for (let i = title.range[0]; i < title.range[1]; i++) {
									var r = a[i]
									if (!r.includes(title.title)) {
										a[i] = title.title + '/' + a[i]
									}
								}
							})
							for (var r in a) {
								...
								if (titles[titleIndex].hasPush && r > titles[titleIndex].range[1]) {
									titleIndex = Math.min(titles.length - 1, titleIndex + 1)
								}

								var title = a[r].split("/")[0]


								if (!titles[titleIndex].hasPush && title === titles[titleIndex].title) {
									titles[titleIndex].hasPush = true
									n.push("<span style='margin-left:6px'> " + title + " </span><br/>")
								}

								if (a[r] === "hr") {
									n.push("<br/>")
								} else {
									var tips = a[r].split("/")[1].split(".")[0]
									a.hasOwnProperty(r) && !!f.default.build(r) && n.push('<i value="' + r + '" title="' + tips + '" >' + f.default.build(r, r) + "</i>");
								}
							}
							t.html(n.join("")),
								s = !0,
					...
					var j = function () {
						...
						, z = function (t, n, r) {
							...
							// 评论加载完毕
							function scrollAndHighlightByHash() {
								...
								if (hash) {
									...
									if (element) {
										...
										// 用 IntersectionObserver 监听何时进入视口
